...
class ProvisionTest(TestCase):
   ...
   def testSerialChangedUnchanged(self):
      lc = getFirstLinecard(self.sup)
      assert not self.manifest.serialChanged(lc)

   def testSerialChangedAbsent(self):
      lc = getFirstLinecard(self.sup)
      cardName = getCardName(lc)
      del self.manifest.data['linecards'][cardName]
      assert self.manifest.serialChanged(lc)
   ...
   def testSetLinecardUnprovisioned(self):
      lc = getFirstLinecard(self.sup)
      cardName = getCardName(lc)
      self.manifest.setLinecardUnprovisioned(lc, lc.slot.getEeprom())
      assert cardName in self.manifest.data['linecards']
      assert self.manifest.data['linecards'][cardName]['serial'] == \
         serialForLinecard(0)
      assert not self.manifest.data['linecards'][cardName]['provisioned']
