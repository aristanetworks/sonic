...
{
    return DIV_ROUND_CLOSEST((long)temp * 1000, 256);
}
...
static irqreturn_t max31732_irq_handler(s32 irq, void *data)
{
    ...
    if (ret) {
        dev_err(dev, "regmap_read PRIM_HIGH_STATUS failed: %d\n", ret);
        return IRQ_NONE;
    }
    ...
    if (ret) {
        dev_err(dev, "regmap_read PRIM_LOW_STATUS failed: %d\n", ret);
        return IRQ_NONE;
    }
    ...
    if (ret) {
        dev_err(dev, "regmap_read SECOND_HIGH_STATUS failed: %d\n", ret);
        return IRQ_NONE;
    }
    ...
    if (ret) {
        dev_err(dev, "regmap_read SECOND_LOW_STATUS failed: %d\n", ret);
        return IRQ_NONE;
    }
